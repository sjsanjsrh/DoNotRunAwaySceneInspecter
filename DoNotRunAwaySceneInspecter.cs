...
namespace DoNotRunAwaySceneInspecter
{
    ...
    class InspectorHelper_OpenInspectorButton_Patch
    {
        public static bool Prefix(
              ...
              Slot source = null,
              bool openWorkerOnly = false)
        {
            if (!DoNotRunAwaySceneInspecter.Enabled) { return true; }
            ...
            else
            {
                ...
                if (slot2 != null)
                {
                    ...
                    //slot2.Position_Field.TweenTo(slot2.LocalPosition + (slot2.LocalRotation * float3.Forward * 0.1f), 0.2f);
                    slot1.Position_Field.TweenTo(slot1.LocalPosition + (slot1.LocalRotation * float3.Backward * DoNotRunAwaySceneInspecter.Distance), 0.2f);
                }
                else
